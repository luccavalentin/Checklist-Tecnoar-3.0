-- Conta da equipe também pode ser cliente no app SOS (a mesma pessoa usa o
-- app como mecânico e, trocando de modo, como cliente — com o próprio
-- cadastro de cliente e os próprios veículos).
--
-- Antes, a conta da equipe era barrada no cadastro de cliente e as funções
-- decidiam o papel só pela conta: um admin que abrisse SOS pelo app caía no
-- fluxo da central ("Informe o cliente"). Agora:
-- - `sos_registrar_conta` aceita funcionário ativo (o pedido de acesso
--   pendente continua bloqueado, como antes);
-- - `sos_meu_papel` devolve, para a equipe, também a conta de cliente
--   (`cliente`, ou null) — o app mostra a troca de modo;
-- - no chamado, o papel sai do vínculo com ele (`sos_papel_no_chamado`): o
--   dono do pedido age como cliente, o mecânico dele como mecânico;
-- - `sos_abrir_chamado` só segue o fluxo da central quando ela informa o
--   cliente — sem `cliente_id`, quem tem conta de cliente pede para si.

-- ────────────────────────────────────────────── papel dentro de um chamado
