-- Etapa feita sem sinal vale na hora do toque, não na hora em que o sinal
-- voltou. A fila do app do mecânico manda `registrado_no_aparelho_em` quando a
-- ação sai com mais de 1 min de atraso; sem isso, um "cheguei" guardado por
-- 20 min na serra estourava o SLA do contrato e inflava o deslocamento.
--
-- O relógio do aparelho não é de confiança total, então a hora dele só vale
-- se: for passado (nunca futuro), tiver no máximo 12 h e não ficar antes da
-- etapa anterior. Fora disso, vale a hora do servidor, como sempre.
