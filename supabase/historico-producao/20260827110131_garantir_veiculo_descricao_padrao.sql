-- Migração aplicada em produção (projeto zdhebeqlhynffxfmedvj), versão 20260827110131.
-- Exportada de supabase_migrations.schema_migrations: é o registro do que rodou. Não edite.

-- `veiculos.descricao` é obrigatória. Quando o atendente só tem a placa na
-- mão, a descrição nasce com a própria placa — a identificação real entra
-- depois, no cadastro do veículo, sem travar a abertura do checklist.
...
declare
  ...
  v_descricao text := nullif(btrim(coalesce(p_veiculo_descricao, '')), '');
...
begin
  ...
  if v_veiculo is null and v_placa <> '' then
    ...
    if v_veiculo is null then
      ...
      insert into veiculos (placa, descricao, tipo, cliente_id)
      values (
        upper(btrim(p_placa)),
        coalesce(v_descricao, 'Veículo ' || upper(btrim(p_placa))),
