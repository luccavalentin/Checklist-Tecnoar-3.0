  ...
  return (
    <header
      className={cn(
        ...
        transparente ? 'bg-transparent' : 'border-b border-line/70 bg-canvas/95 text-ink backdrop-blur-md',
        className,
      )}
    >
      <div className="mx-auto flex min-h-[3.75rem] w-full max-w-xl items-center gap-2 px-3 pr-[max(0.75rem,env(safe-area-inset-right))] pl-[max(0.75rem,env(safe-area-inset-left))]">
        {voltar && (
          <button
            ...
            className="-ml-1 flex size-11 shrink-0 items-center justify-center rounded-full text-ink transition-colors hover:bg-surface-2 active:bg-surface-2"
          >
            <ChevronLeft className="size-6" />
          </button>
        )}
        <div className={cn('min-w-0 flex-1', !voltar && 'pl-1')}>
          {titulo && <h1 className="truncate font-display text-[19px] leading-tight font-bold text-ink">{titulo}</h1>}
...
export function CartaoApp({
  ...
  return (
    <Comp
      ...
      onClick={onClick}
      className={cn(
        'rounded-[1.25rem] border border-line bg-surface p-4 text-left text-ink shadow-[0_1px_2px_rgb(11_28_51/0.04)]',
...
const VARIANTES: Record<VarianteApp, string> = {
  primario: 'bg-accent text-white shadow-accent hover:bg-accent-hover',
  sos: 'bg-[#ff6600] text-white shadow-[0_1px_2px_rgb(11_28_51/0.12),0_10px_22px_-12px_rgb(255_102_0/0.7)] hover:bg-[#cc5200]',
  escuro: 'bg-[#0D1C33] text-white hover:bg-[#002061] dark:bg-[#002061]',
  neutro: 'border border-line-strong bg-surface text-ink hover:bg-surface-2',
...
const TAMANHOS: Record<TamanhoApp, string> = {
  md: 'min-h-12 px-5 text-[15px] gap-2 rounded-[0.95rem]',
  lg: 'min-h-14 px-6 text-[16px] gap-2.5 rounded-2xl',
  xl: 'min-h-16 px-7 text-[17px] gap-3 rounded-[1.15rem]',
}
...
export const BotaoApp = forwardRef<HTMLButtonElement, BotaoAppProps>(function BotaoApp(
  ...
  return (
    <button
      ...
      className={cn(
        'inline-flex items-center justify-center font-display font-bold transition-[transform,background-color,border-color] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none disabled:active:scale-100',
        VARIANTES[variante],
        TAMANHOS[tamanho],
        largo && 'w-full',
        className,
...
export function Folha({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[80] flex items-end justify-center sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-label={typeof titulo === 'string' ? titulo : undefined}>
      ...
      <div className="sos-sobe relative flex max-h-[92dvh] w-full max-w-lg flex-col overflow-hidden rounded-t-[1.75rem] border border-line bg-surface text-ink shadow-[0_-10px_40px_-20px_rgb(8_24_48/0.45)] sm:rounded-[1.75rem]">
...
export function BarraNavegacao({ itens, centro, className }: { itens: ItemBarra[]; centro?: ReactNode; className?: string }) {
  ...
  return (
    <nav aria-label="Navegação" className={cn('sos-bottom-nav fixed z-40', className)}>
      <div className="mx-auto flex h-[4.25rem] max-w-xl items-stretch justify-around px-1">
        {esquerda.map((i) => (
          <ItemNavegacao key={i.rota} item={i} />
        ))}
        {centro && <div className="relative flex w-20 shrink-0 items-start justify-center">{centro}</div>}
        {direita.map((i) => (
          <ItemNavegacao key={i.rota} item={i} />
        ))}
      </div>
...
>(function CampoApp({ rotulo, erro, dica, icone: Icone, direita, className, id, ...props }, ref) {
  ...
  return (
    <div className={cn('flex flex-col gap-1.5', className)}>
      ...
      <div
        className={cn(
          'sos-field flex min-h-[3.25rem] items-center gap-2.5 rounded-2xl px-3.5 transition-colors focus-within:border-accent focus-within:ring-4 focus-within:ring-accent/15',
          erro && '!border-crit',
        )}
      >
  ...
  return (
    <div className={cn('flex flex-col gap-1.5', className)}>
      ...
      <textarea
        ...
        rows={3}
        className={cn(
          'sos-field min-h-24 w-full resize-y rounded-2xl px-3.5 py-3 text-[16px] leading-relaxed text-ink outline-none transition-colors placeholder:text-ink-3/70 focus:border-accent focus:ring-4 focus:ring-accent/15',
          erro && '!border-crit',
        )}
        {...props}
      />
...
export function Faixa({ tom = 'atencao', children, icone: Icone }: { tom?: 'atencao' | 'critico' | 'info' | 'ok'; children: ReactNode; icone?: LucideIcon }) {
  return (
    <div
      role="status"
      className={cn(
        'flex items-start gap-2.5 rounded-2xl border px-3.5 py-3 text-[13.5px] leading-snug',
