...
import { mascaraPlaca, mascaraTelefone, somenteDigitos } from '@/lib/formatos'
import { useToast } from '@/componentes/ui/Toast'
import { sosBuscarClienteCampo, sosMecanicoAbrirChamado } from '@/sos/api'
import {
  buscarLugar,
  distanciaKm,
  enderecoDoPonto,
...
import type { ClienteCampo, HomeMecanico, NovoChamadoMecanico as PedidoChamado, OcorrenciaSOS } from '@/sos/tipos'
...
import { AreaM, BotaoM, CampoM, CartaoM, EsqueletoM, OpcaoM, RodapeAcao, RotuloM, SecaoM, SeloM, TelaM, TopoM, VazioM } from './ui'
...
const normalizarPlaca = (v: string | null | undefined) => (v ?? '').toUpperCase().replace(/[^A-Z0-9]/g, '')
    ...
    <TelaM comBarra={false} className="pb-[calc(2rem+env(safe-area-inset-bottom))]">
      {atual && (
        <CartaoM className="flex flex-col gap-3 border-2 border-accent/50">
          ...
          <BotaoM variante="laranja" tamanho="lg" largo onClick={() => aoEscolher(atual.cliente, atual.veiculo)}>
      ...
      {!online ? (
        <AvisoLinha icone={WifiOff} tom="ambar">
          Sem internet agora. A busca e a abertura do chamado precisam de conexão — tente de novo quando o sinal voltar.
        </AvisoLinha>
      ) : !ativa ? (
        <>
        ...
        <VazioM
          ...
          texto="Confira a placa ou o telefone. Se for a primeira vez, cadastre como cliente novo — a central completa depois."
          acao={
...
function FormularioCliente({
  ...
  const erroPlaca = placaN && !PLACA_VALIDA.test(placaN) ? 'Placa inválida. Ex.: ABC1D23 (Mercosul) ou ABC1234.' : null
  const temVeiculo = !!placaN || !!modelo.trim()

  function continuar() {
    setTentou(true)
    if (erroNome || erroTel || erroPlaca) return
    const c: ClienteEscolhido = existente ?? { id: null, nome: nome.trim(), telefone: mascaraTelefone(dig) }
      ...
      <TelaM comBarra={false} className="pb-[calc(8rem+env(safe-area-inset-bottom))]">
        {existente ? (
          ...
          <>
            ...
            <AvisoLinha icone={UserPlus} tom="ciano">
              Cliente novo: a central recebe um aviso para completar o cadastro. Se o celular já existir, o sistema usa o cadastro que já tem.
            </AvisoLinha>
            <CampoM
        ...
        <SecaoM titulo="Veículo">
          <CampoM
            ...
            dica="Mercosul ou modelo antigo. Com a placa, a OS já pode sair junto."
          />
          <CampoM
            ...
            autoComplete="off"
            maxLength={120}
            dica={!placaN && modelo.trim() ? 'Sem placa, o modelo vai na descrição do chamado.' : undefined}
          />
        </SecaoM>
      </TelaM>
      <RodapeAcao teclado={teclado}>
        <BotaoM variante="laranja" tamanho={teclado ? 'lg' : 'xl'} largo onClick={continuar}>
          {temVeiculo ? 'Continuar' : 'Continuar sem veículo'}
        </BotaoM>
      </RodapeAcao>
    </>
  )
...
function ErroCarregar({ mensagem, tentando, aoTentar }: { mensagem: string; tentando: boolean; aoTentar: () => void }) {
  return (
...
function AvisoLinha({ icone: Icone, tom, children }: { icone: LucideIcon; tom: 'ambar' | 'ciano'; children: ReactNode }) {
  return (
    <p
      className={cn(
